...
import { Settings, ChevronDown, FileText, UploadCloud, Copy, CheckCircle } from "lucide-react";
...
type HeaderProps = EditorHeaderProps | ViewerHeaderProps | SimpleHeaderProps;
...
export default function Header(props: HeaderProps) {
  ...
  if (props.variant === "viewer") {
    const { lineCount, wordCount, charCount, copied, onCopyClick } = props;
    ...
    return (
      <header className="flex items-center justify-between border-b border-zinc-800 px-5 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onCopyClick}
            className="flex items-center gap-1.5 border border-zinc-700 px-3 py-1.5 text-xs text-zinc-400 transition-colors hover:border-zinc-500 hover:text-zinc-300"
          >
            {copied ? (
              <>
