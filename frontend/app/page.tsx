...
export default function PasteView({ params }: PasteViewProps) {
  ...
  return (
    ...
    >
      {/* Top Bar - Tritan Modernized */}
      <motion.header
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
        className="relative z-20 flex items-center justify-between px-8 py-4 border-b border-[rgba(139,92,246,0.15)] bg-[rgba(10,10,18,0.8)] backdrop-blur-md"
      >
            ...
            <>
              ...
              >
                {copied ? (
                  ...
                  <>
                    <Copy className="w-3.5 h-3.5 text-[#8b5cf6]" /> Copy
                  </>
                )}
              </button>
              <a
                href="/"
                className="flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-lg bg-[rgba(139,92,246,0.22)] border border-[rgba(139,92,246,0.4)] text-white hover:bg-[rgba(139,92,246,0.32)] transition-all shadow-[0_0_20px_rgba(139,92,246,0.1)_inset]"
              >
                + New Paste
              </a>
            </>
      ...
      >
        ...
        {isPasswordProtected && !isLoading && (
          <div className="flex-1 flex items-center justify-center p-6">
            ...
            >
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-lg bg-[#8b5cf6] text-white font-semibold text-sm hover:bg-[#7c3aed] transition-colors"
              >
                Unlock
              </button>
            </motion.form>
        ...
        {error && !isPasswordProtected && !isLoading && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center space-y-3">
              ...
              <a
                href="/"
                className="text-[#8b5cf6] hover:text-[#c7c9f1] text-xs underline underline-offset-2 transition-colors"
              >
                ← back to home
              </a>
            </div>
        ...
        {paste && !isLoading && !isPasswordProtected && (
          <>
            <div
              className="hidden sm:block text-right text-[11px] leading-7 text-[#52525b] px-5 pt-5 border-r border-[rgba(139,92,246,0.1)] bg-[rgba(10,10,18,0.4)] min-w-[4rem]"
              aria-hidden
            >
      ...
      {/* Footer Status Bar */}
      <footer className="relative z-20 flex items-center justify-between px-6 py-2 border-t border-[rgba(139,92,246,0.15)] bg-[#09090b]/80 backdrop-blur-md text-[10px] text-[#52525b] uppercase tracking-widest font-mono">
